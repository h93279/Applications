...
namespace LifeNumber
{
    public partial class Form1 : Form
    {
        ...
        {
            date = dateTimePicker1.Value.ToString("yyyyMMdd");
            ymd = date.ToCharArray();
            int ymdSum = 0, ymdSum2 = 0;
            for (int i = 0; i < ymd.Length; i++)
            {
                ymdSum = ymdSum + int.Parse(ymd[i].ToString());  //個數加總   
            }
            if (ymdSum > 9)               //如果數字大於9
            {
                char[] ymd2 = ymdSum.ToString().ToCharArray();
                for (int j = 0; j < 2; j++)
                {
                    ymdSum2 = ymdSum2 + int.Parse(ymd2[j].ToString());
                }
                ymdSum = ymdSum2;
                label3.Text += "你的生命靈數是：" + ymdSum.ToString() + "\n\n";
            }
            else
            {
                label3.Text = "你的生命靈數是："+ ymdSum.ToString() + "\n\n";
            ...
            string zzs = ZodiacSign(dtp.Value);    //星座值(string)        //label3.Text += zzs;
            string newresult = string.Empty;
            if (zzs == "魔羯座")
            {
                newresult = CreateCapricorn()[ymdSum];

            }
            else if (zzs == "水瓶座")
            {
                newresult = CreateAquarius()[ymdSum];
            }
            else if (zzs == "雙魚座")
            {

                newresult = CreatePieces()[ymdSum];
            }
            else if (zzs == "牡羊座")
            {
                newresult = CreateAries()[ymdSum];
            }
            else if (zzs == "金牛座")
            {
                newresult = CreateTaurus()[ymdSum];
            }
            else if (zzs == "雙子座")
            {
                newresult = CreateGemini()[ymdSum];
            }
            else if (zzs == "巨蠍座")
            {
                newresult = CreateCancer()[ymdSum];
            }
            else if (zzs == "獅子座")
            {
                newresult = CreateLeo()[ymdSum];
            }
            else if (zzs == "處女座")
            {
                newresult = CreateVirgo()[ymdSum];
            }
            else if (zzs == "天秤座")
            {
                newresult = CreateLibra()[ymdSum];
            }
            else if (zzs == "天蠍座")
            {
                newresult = CreateScorpio()[ymdSum];
            }
            else if (zzs == "射手座")
            {
                newresult = CreateSagittarius()[ymdSum];
            }
        ...
        public string ZodiacSign(DateTime datemmdd)   //計算星座區間
        {
            date = dateTimePicker1.Value.ToString("MMdd");
            int[] mmddstr = { 0101, 0120, 0220, 0321, 0421, 0521, 0622, 0723, 0823, 0923, 1023, 1121, 1222, 1231 };
            string[] zs = { "魔羯座" ,"水瓶座", "雙魚座", "牡羊座", "金牛座", "雙子座", "巨蟹座", "獅子座", "處女座", "天秤座", "天蠍座", "射手座", "魔羯座" };
            string zodiaSign = string.Empty;
            for (int i = 0; i < mmddstr.Length - 1; i++)
            {
                if (mmddstr[i] <= int.Parse(date) && mmddstr[i + 1] > int.Parse(date))
                {
                    zodiaSign = zs[i].ToString();
                    break;
                }
            }
            return zodiaSign;
